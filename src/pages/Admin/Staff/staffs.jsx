...
import StaffModalDetail from '~/pages/Admin/Staff/staffModalDetail';
import { useDispatch, useSelector } from 'react-redux';
import { filteredStaffsSelector } from '~/redux/selector/userSelector';
import { userActions } from '~/redux/slice/userSlice';
import Loading from '~/components/shared/Loading/loading';

const Staffs = () => {
    const token = useSelector((state) => state.auth.auth.access_token);
    const { staffs, page, limit, total } = useSelector((state) => state.user);
    ...
    const filteredStaffs = useSelector((state) =>
        filteredStaffsSelector(state, filterInput, activeFilter)
    );
    ...
    const handlePageChange = (page, limit) => {
        dispatch(userActions.fetchStaffs({ token, page, limit }));
    };
    ...
    const fetchData = useCallback(() => {
        ...
        try {
            ...
            toast.error('Lấy dữ liệu nhân viên thất bại');
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [token, page, limit, dispatch, staffs.data.length, totalPages]);
