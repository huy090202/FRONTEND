...
import { selectFilteredParts } from '~/redux/selector/partSelector';

const Parts = () => {
    const token = useSelector((state) => state.auth.auth.access_token);
    const { partPrivates, loading, page, limit, total } = useSelector((state) => state.part);
    ...
    const filteredParts = useSelector((state) =>
        selectFilteredParts(state, filterInput, activeFilter)
    );
    ...
    const handlePageChange = (page, limit) => {
        dispatch(partActions.fetchPartPrivates({ token, page, limit }));
    };
    ...
    const handleDeletePart = useCallback(
        (id) => {
            ...
            dispatch(partActions.deletePart({ token, id }));
        },
    ...
    const fetchData = useCallback(() => {
        ...
        try {
            ...
            toast.error('Lấy dữ liệu phụ tùng thất bại');
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [token, page, limit, dispatch, partPrivates.length, totalPages]);
