...
import TechnicianModalDetail from '~/pages/Admin/Technician/technicianModalDetail';
import { useDispatch, useSelector } from 'react-redux';
import { filteredTechniciansSelector } from '~/redux/selector/userSelector';
import { userActions } from '~/redux/slice/userSlice';
import Loading from '~/components/shared/Loading/loading';

const Technicians = () => {
    const token = useSelector((state) => state.auth.auth.access_token);
    const { techs, page, limit, total } = useSelector((state) => state.user);
    ...
    const filteredTechs = useSelector((state) =>
        filteredTechniciansSelector(state, filterInput, activeFilter)
    );
    ...
    const handlePageChange = (page, limit) => {
        dispatch(userActions.fetchTechs({ token, page, limit }));
    };
    ...
    const fetchData = useCallback(() => {
        ...
        try {
            ...
            toast.error('Lấy dữ liệu kỹ thuật viên thất bại');
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [token, page, limit, dispatch, techs.data.length, totalPages]);
        ...
        <Fragment>
            ...
            <div className='flex flex-col w-full'>
                <div className='flex items-center justify-between my-10'>
                    <h1 className='text-4xl font-bold '>Kỹ thuật viên</h1>
                    <button
