...
import { filteredCashiersSelector } from '~/redux/selector/userSelector';
import { userActions } from '~/redux/slice/userSlice';
import Loading from '~/components/shared/Loading/loading';

const Cashiers = () => {
    const token = useSelector((state) => state.auth.auth.access_token);
    const { cashiers, page, limit, total } = useSelector((state) => state.user);
    ...
    const filteredCashiers = useSelector((state) =>
        filteredCashiersSelector(state, filterInput, activeFilter)
    );
    ...
    const handlePageChange = (page, limit) => {
        dispatch(userActions.fetchCashiers({ token, page, limit }));
    };
                ...
                Cell: ({ row }) => (
                    <Switch
                        ...
                        onChange={async (checked) => {
                            ...
                            try {
                                dispatch(
                                    userActions.updateCashierStatus({
                                        token,
                                        id: row.original.id,
                                        active: checked
                                    })
    ...
    const fetchData = useCallback(() => {
        ...
        try {
            ...
            toast.error('Lấy dữ liệu thu ngân thất bại');
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [token, page, limit, dispatch, cashiers.data.length, totalPages]);
        ...
        <Fragment>
            ...
            <div className='flex flex-col w-full'>
                <div className='flex items-center justify-between my-10'>
                    <h1 className='text-4xl font-bold '>Thu ngân</h1>
                    <button
